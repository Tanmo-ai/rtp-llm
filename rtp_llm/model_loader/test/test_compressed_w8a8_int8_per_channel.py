...
class TestPerChannelFp8TemplateDefaults(unittest.TestCase):
    """Guard the FP8 defaults that the INT8 loader subclasses override.

    The INT8 loader reuses PerChannelFp8Weight by overriding two class
    attributes. If the FP8 defaults ever change, the FP8 load path silently
    changes behaviour, so pin them here.
    """

    def test_fp8_defaults_unchanged(self):
        from rtp_llm.model_loader.per_channel_fp8_quant_weight import (
            PerChannelFp8Weight,
        )

        self.assertIs(PerChannelFp8Weight.weight_dtype, torch.float8_e4m3fn)
        self.assertTrue(PerChannelFp8Weight.apply_fp8_device_conversion)

    def test_int8_overrides_differ_from_fp8(self):
        from rtp_llm.model_loader.compressed_w8a8_int8_per_channel_weight import (
            CompressedW8A8Int8PerChannelWeight,
        )

        self.assertIs(CompressedW8A8Int8PerChannelWeight.weight_dtype, torch.int8)
        self.assertFalse(
            CompressedW8A8Int8PerChannelWeight.apply_fp8_device_conversion
        )
