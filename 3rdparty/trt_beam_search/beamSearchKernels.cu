...
// Host wrappers below keep the kernel launch in the same TU as the kernel
// definition so nvcc 13's local-linkage instantiations can be resolved
// (see beamSearchKernels.h).
namespace
{
template <typename T>
__global__ void addCumLogProbsKernel(T* __restrict pStage1LogProbs, float const* __restrict cumLogProbs,
...
template <typename T>
void launchAddCumLogProbs(T* pStage1LogProbs, float const* cumLogProbs, FinishedState const* finished,
    int const* endIds, float const* diversityRates, runtime::SizeType32 const* batchSlots, size_t nBS,
    size_t nBMIn, size_t nBMOut, int nThread, cudaStream_t stream)
{
...
template void launchAddCumLogProbs<float>(float*, float const*, FinishedState const*, int const*, float const*,
    runtime::SizeType32 const*, size_t, size_t, size_t, int, cudaStream_t);

template void launchAddCumLogProbs<half>(half*, float const*, FinishedState const*, int const*, float const*,
    runtime::SizeType32 const*, size_t, size_t, size_t, int, cudaStream_t);
